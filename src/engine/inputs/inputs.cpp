...
void frametech::engine::inputs::CursorHandler::poll(bool blank) noexcept
{
    ...
    if (m_positions.empty())
        return;
    // Handle
    std::tuple<float, float> cursor_position = m_positions.front();
...
void frametech::engine::inputs::CursorHandler::addMove(const float xpos, const float ypos) noexcept
{
    ftstd::mutex::ScopedMutex guard(&m_lock, (char*)"CursorHandler::addMove");
    m_positions.push_back(std::tuple<float, float>(xpos, ypos));
}
