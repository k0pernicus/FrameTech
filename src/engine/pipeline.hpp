...
namespace FrameTech
{
    ...
    public:
        Pipeline();
        ~Pipeline();
        /// Read each shader file passed as parameter, if those exist.
        /// **Warning**: this function is **not** data-race conditons bullet-proof.
        /// TODO: Real return type.
        void create(const char* vertex_shader_filepath, const char* fragment_shader_filepath);

    private:
        /// Returns the size, as a `uint64_t` type, of a file located at `filepath`.
        /// If the file does not exists, or can't be read, return a `nullopt` value.
        /// **Warning**: this function is **not** data-race conditons bullet-proof.
        std::optional<uint64_t> fileSize(const char* filepath);
        /// Read the content of a file, located at `filepath`, and put the content of it
        /// in `buffer`.
